...
exports.loadData = function(req, res, next){
  	...
  	var user = '';
	var username = '';
	var userLoginURI = "https://api.instagram.com/oauth/authorize/?client_id=" + process.env.CLIENT_ID + "&redirect_uri=http://" + process.env.ADDRESS + process.env.PORT + "/photos&response_type=code";
	...
	if (req.url !== undefined && urlParser.parse(req.url).query !== null) {
		...
		var sendData = {
			...
			'redirect_uri' : 'http://' + process.env.ADDRESS + process.env.PORT + '/photos',
			'code' : code
		}
		httpsLoader.httpsPost('/oauth/access_token', sendData, function (parsed, err) { 
			if (err) {
				...
				res.redirect(process.env.ADDRESS + process.env.PORT + '/photos', {"message": userMessage, 'hrefAddress': userLoginURI});
			}
		});
	}
	else {
		res.render('./photos', {"message": userMessage, "hrefAddress": userLoginURI});
	}
};
